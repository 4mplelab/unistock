...
4. ブラウザで`http://localhost:3000`を開く →「デモを試す」ボタン → セットアップウィザードでショップを作成する(「デモ用のサンプルデータを投入する」にチェックすると、部品・BOM・注文・発注等が入った状態で試せる)

## 実際にBASEと連携して運用する場合

上記と同じ2ファイルを使います(すでにデモモードで試し済みなら、新しく用意せず同じ`.env`を編集するだけでよい)。

1. `.env`を編集する
   - `DEMO_MODE=false`(未指定時のデフォルト)
   - `BASE_CLIENT_ID` / `BASE_CLIENT_SECRET`: [BASE Developers](https://developers.thebase.in)で取得
   - **UniStock自体へのログイン設定(Google/Microsoft/GitHub/Xのいずれか1つ以上)**: デモモードと違い、通常モードでは起動後すぐログイン画面になるため、先にこの設定を済ませておく必要があります。  
     設定方法は[ログイン方法の設定](docs-site/src/content/docs/admin-guide/login-setup.mdx)を参照してください
...
2. ブラウザで`http://localhost:3000`を開き、手順1で設定したアカウントでログインする。  
   (最初にログインした人が自動的に管理者になります)  
   ログイン後、セットアップウィザードでショップを作成する。  
